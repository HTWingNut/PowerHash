...
# powerhasha256.ps1 by HTWingNut 21 Jan 2024
...
function GenerateHash {
    ...
    Write-Output "Calculating $filesize ($([int64]($filesize/1MB)) MB) $SecondsElapsed seconds" >> $timelog
    ...
    Write-Host ""
    Write-Host "**** Files in Folder: $numfiles Size: $filesize $([int]($filesize/1MB)) MB Seconds: $SecondsElapsed"
    Write-Host "**** Files Excluded: $excludecount"
    Write-Host "**** Total Files Hashed: $hashcount Size: $totalhashbytes $([int]($totalhashbytes/1MB)) MB"
    Write-Host ""
...
function UpdateHash {
    ...
    Write-Output "" | Out-File -Encoding UTF8 -Append -FilePath "$updatedlog"
    ...
    if ($excludeyn) { Write-Output "**** Exclusions List Updated:" | Out-File -Encoding UTF8 -Append -FilePath "$historylog" }
...
function Scrub {
    ...
    $folderUniqueObj = @()
    $foldervScrubObj = @()
    $foldervScrubObj = Compare-Object @( $folderObjects | Select-Object ) @( $FolderScrubObj | Select-Object) -Property File -PassThru | Where-Object SideIndicator -eq '<='
    $folderUniqueObjCount = ( $foldervScrubObj | Measure-Object ).Count
    Write-Host "count: $folderuniqueobjcount"
    if ($hashnew) { Write-Host "`nHashing $folderUniqueObjCount New Files Found in '$filepath_base':" }

    $folderUniqueObj = $foldervScrubObj | ForEach-Object {
        $fileInfo = $_
        $fhash = $_.hash
        $fname = $_.file
        $fsize = $_.size
        ...
        $count++
            ...
            if ($excludeFolderString -ne "") {
                if ($FileParent -match $excludeFolderList) {
                    ...
                    Write-Host "$count of $folderUniqueObjCount ** EXCLUSION: [PATH: $($matches -join ',')] $fname"
                    $FolderExcludedcount++ 
                    $skip = $true
                }
            }
            ...
            if (!$skip) { 
                ...
                if ($hashnew) {
                    Write-Host "$count of $folderUniqueObjCount $fsize $fname" -NoNewline
                    $filehash = ""
                    $filehash = (Get-FileHash -LiteralPath $fullpath -Algorithm $algo).Hash
                    $folderUniqueCount++; $folderUnique += "$filehash $fname $fsize $fdate`n"
                    Write-Host "`r$count of $folderUniqueObjCount $fsize $fname"
                    [PSCustomObject]@{
                        'Hash' = $filehash
                        'File' = $fname
                        'Size' = $fsize
